...
`allowed_pe_hosts` lists every configured cluster that is answering: `NUTANIX_ALLOWED_PE_HOSTS` in order, then any host configured only in `NUTANIX_PE_CREDENTIALS`, minus any host whose Prism did not answer an unauthenticated probe (results cached ~30 s). Build the cluster picker from it. `default_pe_host` is the first answering host, or `null` when clusters are configured but none answer — show that as "no cluster reachable" rather than prefilling a host.
